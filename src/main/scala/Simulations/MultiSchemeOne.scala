...
object MultiSchemeOne {
  private val map: MapperUtil = new MapperUtil
  private val reduce: ReducerUtil = new ReducerUtil
  private val log: Logger = LoggerFactory.getLogger(MultiSchemeOne.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    var k: Int = 0
    while ( {
      k <= dataCenterConfig.getInt("MapperCloudlet.count")
    }) {
      broker1.bindCloudletToVm(createMappers(broker1.getId).get(k).getCloudletId, vmUtilMap.getVmList.get(0).getId)
  ...
  private def printCloudletList(list: util.List[Cloudlet]): Unit = {
    ...
    val indent: String = "    "
    Log.printLine()
    Log.printLine(indent+"========== OUTPUT MULTI SCHEME ONE =========="+indent)
