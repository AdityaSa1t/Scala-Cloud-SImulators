...
object SchemeFive {
  ...
  private val log: Logger = LoggerFactory.getLogger(SchemeFive.getClass)
  ...
  private def createCloudlets(brokerId: Int): util.List[Cloudlet] = {
    val cloudletUtilization_1: UtilizationModel = new UtilizationModelStochastic
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Creating a Data Center - Time Shared over subscription - VM and Space Shared- Cloudlet
    val DC_Util: DataCenterUtil = new DataCenterUtil
    val DC0: Datacenter = DC_Util.createDataCenterTimeSharedSubscription(dataCenterConfig.getString("DataCenter.name1")) //time-shared subscription VM scheduling
  ...
  private def printCloudletList(list: util.List[Cloudlet]): Unit = {
    ...
    val indent: String = "    "
    Log.printLine()
    Log.printLine(indent+"========== OUTPUT SCHEME FIVE =========="+indent)
