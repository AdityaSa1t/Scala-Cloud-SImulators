...
import org.cloudbus.cloudsim.{Cloudlet, CloudletSchedulerTimeShared, Datacenter, DatacenterBroker, Log, UtilizationModel, UtilizationModelFull}
...
object SchemeThree {
  ...
  private val log: Logger = LoggerFactory.getLogger(SchemeThree.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Creating a Data Center - Space Shared- VM and Time Shared- Cloudlet
    ...
    try broker = new DatacenterBroker(dataCenterConfig.getString("DataCenter.name1"))
      ...
      case e: Exception =>
        e.printStackTrace()
        log.error("Master Cloudlet created.")
    }
  ...
  private def printCloudletList(list: util.List[Cloudlet]): Unit = {
    ...
    val indent: String = "    "
    Log.printLine()
    Log.printLine(indent+"========== OUTPUT SCHEME THREE =========="+indent)
