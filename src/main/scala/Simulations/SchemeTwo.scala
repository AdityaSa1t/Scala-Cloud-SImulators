...
import org.cloudbus.cloudsim.{Cloudlet, CloudletSchedulerTimeShared, Datacenter, DatacenterBroker, Log, UtilizationModel, UtilizationModelFull, Vm}
...
object SchemeTwo{
  ...
  private val log: Logger = LoggerFactory.getLogger(SchemeTwo.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Creating a Data Center - Time Shared Policy
  ...
  private def printCloudletList(list: util.List[Cloudlet]): Unit = {
    ...
    val indent: String = "    "
    Log.printLine()
    Log.printLine(indent +"========== OUTPUT SCHEME TWO =========="+ indent)
    ...
    while ( {
      ...
      if (cloudlet.getCloudletStatus == Cloudlet.SUCCESS) {
        ...
        //Log.printLine(dft.format(cloudlet.getExecStartTime()));m
